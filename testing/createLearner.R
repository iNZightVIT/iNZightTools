createLearner <- function(task, learner.name,
                          predict.type = "response", # predict.type is dynamic in the GUI, options (c("response", "probability") for classif/surv tasks OR just "response" for regr tasks) depend on task.type
  ...
  cl = sprintf("%s.%s", task$task.type, learner.name)
