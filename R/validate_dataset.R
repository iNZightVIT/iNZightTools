...
validation_summary <- function(cf) {
  ...
  new.summary$percentfailed <- sprintf("%.2f%%", new.summary$fails / new.summary$total * 100)
...
##' @param id.var Variable name denoting a unique identifier for each observation
...
validation_details <-  function(cf, v, var, id.var, df) {
  ...
  if (n.fails == 0) {
    if (var %in% names(validate::errors(cf))) {
      details.table <- sprintf("Error occurred: \n  %s", validate::errors(cf)[var])
      n.fails <- NA
    } else {
    ...
    if (length(which.expr) > 1) {
      ...
      colnames(inspect.vars)[ncol(inspect.vars)] <- capture.output(print(which.expr))
    }
    
    if (!is.na(id.var)) {
      inspect.vars <- cbind(df[which.failed, id.var, drop = FALSE], inspect.vars)
    }
  ...
  if (var %in% names(validate::warnings(cf))) {
    details.output <- c(details.output, sprintf("Warnings: \n  %s", validate::warnings(cf)))
  }
